...
app.get("/scrape", async (req, res) => {
  ...
  try {
    ...
    if (selectors.type === "recipe") {
      const recipeData = ContentExtractor.extractRecipe($, selectors, hostname);
      
      if (
        recipeData.recipeName === "N/A" &&
      ...
      const newsData = ContentExtractor.extractNews($, selectors, hostname);
      
      if (
        newsData.title === "N/A" &&
        newsData.author === "N/A" &&
        newsData.content.length === 1 &&
        newsData.content[0].text === "N/A"
      ) {
...
// /batch-generate-selectors endpoint
  ...
  for (const url of urls) {
    ...
    try {
      browser = await puppeteer.launch({
        ...
        executablePath: 'C:\\Users\\Ed\\Documents\\Programming\\Angular\\webscraping\\Project\\recipe-app\\server\\chrome\\win64-129.0.6668.100\\chrome-win64\\chrome.exe'
      });
...
//       await page.goto(url, { waitUntil: "networkidle2", timeout: 60000 });
//       try {
//         await page.waitForSelector(selectors.ingredients, { timeout: 15000 });
//         console.log(`Found ingredients selector: ${selectors.ingredients}`);
//       } catch (e) {
//         console.warn(`Ingredients selector ${selectors.ingredients} not found`);
//       }
//       data = await page.content();
//       await browser.close();
//     }
...
// // /batch-generate-selectors endpoint (unchanged)
// app.post("/batch-generate-selectors", async (req, res) => {
//   const { urls, type = "news" } = req.body;
//   if (!urls || !Array.isArray(urls) || urls.length === 0) {
//     return res.status(400).json({ error: "URLs array is required" });
